                        ...
                        <div class="card-body text-center card-body-second">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <tr>
                                        <th scope="col"><label
                                                    for="activityCategory">Category</label></th>
                                        <th scope="col"><label for="name">Name</label></th>
                                        <th scope="col"><label for="description">Description</label>
                                        </th>
                                        <th scope="col"><label for="activityEdit">Edit</label></th>
                                        <th scope="col"><label for="activityDelete">Delete</label>
                                        </th>
                                    ...
                                    @if(count($activities)==0)
                                        <tr>
                                            ...
                                            <td scope="co" colspan="5" style="text-align: center">Sorry No Results
                                                Found.
                                            </td>
                                        </tr>
                                    @endif
                                    @foreach($activities as $activity)
                                        <tr>
                                            ...
                                            <td scope="col">{{$activity->category->name_en}}</td>
                                            <td scope="col">{{$activity->name_en}}</td>
                                            <td scope="col">{{\Illuminate\Support\Str::limit(strip_tags($activity->description_en),180)}}</td>
                                            <td>
                                                <a style="padding: 12px 20px;background-color: #d1d138" href="{{route('edit-activity',['language'=>app()->getLocale(),'idactivity'=>$activity->idactivity])}}"

                                                   class="btn btn-warning" ><i class="fa fa-pencil-square-o"
                                                                              aria-hidden="true"></i></a></td>
                                            <td>
                                                <button style="padding: 12px 20px;" type="button" data-id="{{$activity->idactivity}}"
                                                        class="btn deleteBtn btn-danger"><i class="fa fa-trash "
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.deleteBtn', function () {
            var id = $(this).data("id");
            $.post('{{route('deleteByActivityId',app()->getLocale())}}', {id: id}, function (data) {
                if (data.success) {
                    location.reload();
                }
            });
