                                        ...
                                        <div class="table table-responsive b-0" data-pattern="priority-columns">
                                            <table class="table"
                                                ...
                                                @foreach($victories as $victory)
                                                    <tr>
                                                        <td>{{$victory->name_en}}</td>
                                                        <td>{{\Illuminate\Support\Str::limit(strip_tags($victory->description_en),180)}}</td>
                                                        <td><a style="padding: 12px 20px;background-color: #d1d138"
                                                               href="{{route('edit-victory',['language'=>app()->getLocale(),'victoryId'=>$victory->idvictory])}}"
                                                        ...
                                                        <td>
                                                            <button style="padding: 12px 20px;" type="button"
                                                                    data-id="{{$victory->idvictory}}" id="dVictoryId"
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '#dVictoryId', function () {
            var victoryId = $(this).data("id");

            $.post('deleteByVictoryId', {victoryId: victoryId}, function (data) {
                if (data.success) {
                    location.reload();
                }
            });
