                        ...
                        <div class="card-body text-center card-body-second">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <tr>
                                        <th scope="col"><label
                                                    for="eventCategory">Category</label></th>
                                        <th scope="col"><label for="name">Name</label></th>
                                        <th scope="col"><label for="description">Description</label>
                                        </th>
                                        <th scope="col"><label for="eventEdit">Edit</label></th>
                                        <th scope="col"><label for="eventDelete">Delete</label>
                                        </th>
                                    ...
                                    @foreach($events as $event)
                                        <tr>
                                            ...
                                            <td scope="col">{{$event->category->name_en}}</td>
                                            <td scope="col">{{$event->name_en}}</td>
                                            <td scope="col">{{\Illuminate\Support\Str::limit(strip_tags($event->description_en),180)}}</td>
                                            <td>
                                                <a style="padding: 12px 20px;background-color: #d1d138" href="{{route('edit-event',['language'=>app()->getLocale(),'idevent'=>$event->idevent])}}"

                                                   class="btn btn-warning" ><i class="fa fa-pencil-square-o"
                                                                              aria-hidden="true"></i></a></td>
                                            <td>
                                                <button style="padding: 12px 20px;" type="button" data-id="{{$event->idevent}}"
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.deleteBtn', function () {
            ...
            $.post('deleteByEventId', {eventId: eventId}, function (data) {
                console.log(data);
                if (data.success) {
                    location.reload();
                }
